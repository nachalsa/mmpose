...
class BatchFastVideoProcessor:
        ...
        try:
            ...
            for frame_idx, frame in enumerate(frames):
                ...
                if len(batch_frames) == self.batch_size or frame_idx == len(frames) - 1:
                    ...
                    # GPU 메모리 정리
                    if torch.cuda.is_available() and frame_idx % (self.batch_size * 4) == 0:
                        torch.cuda.empty_cache()
            
            process_time = time.time() - start_time
            fps = actual_frame_count / process_time
            
            print(f"✅ 배치 처리 완료: {actual_frame_count} 프레임 ({process_time:.2f}s, {fps:.1f} FPS)")
    ...
    def _process_frame_batch(self, frames: List[np.ndarray]) -> List[Optional[Tuple[List[List[int]], List[float]]]]:
        """프레임 배치 처리 - 진정한 GPU 배치 추론"""
        results = []
        
        try:
            # GPU 배치 추론을 위한 준비
            batch_crops = []  # 모든 크롭 이미지들
            batch_frame_indices = []  # 각 크롭이 어느 프레임에서 왔는지 추적
            ...
            for frame_idx, frame in enumerate(frames):
                try:
                    ...
                    frame_person_counts.append(len(person_boxes))
            ...
            if batch_crops:
                print(f"🔥 GPU 배치 포즈 추정: {len(batch_crops)}개 크롭")
                
                # 배치 크기로 나누어 처리
                crop_batch_size = min(32, len(batch_crops))  # ONNX 배치 크기 조정
                
                for i in range(0, len(batch_crops), crop_batch_size):
                    batch_slice = batch_crops[i:i+crop_batch_size]
