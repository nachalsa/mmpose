#!/usr/bin/env python3
"""
스트림라인 비디오 처리기 - HDF5 배치 처리용 (WORD + SEN 지원) - NVIDIA GPU 버전
...
class YOLO11LCUDAHybridInferencer:
    """NVIDIA GPU용 YOLO11L + RTMW 하이브리드 추론기"""
    
    def __init__(self, rtmw_config, rtmw_checkpoint, detection_device='cuda', 
                 pose_device='cuda', optimize_for_accuracy=True):
        ...
        if not torch.cuda.is_available():
            self.logger.warning("⚠️ CUDA를 사용할 수 없습니다. CPU로 대체합니다.")
            self.detection_device = 'cpu'
            self.pose_device = 'cpu'
        else:
            self.logger.info(f"✅ CUDA 감지됨: {torch.cuda.get_device_name()}")
            # GPU 메모리 정보
            gpu_memory = torch.cuda.get_device_properties(0).total_memory / 1024**3
            self.logger.info(f"   GPU 메모리: {gpu_memory:.1f} GB")
        
        self._init_models(rtmw_config, rtmw_checkpoint, optimize_for_accuracy)
    
    def _init_models(self, rtmw_config, rtmw_checkpoint, optimize_for_accuracy):
        """모델 초기화"""
        try:
            # YOLO 모델 초기화
            from ultralytics import YOLO
            yolo_model_path = Path(MODELS_DIR) / YOLO_MODEL_CONFIG["filename"]
            
            if yolo_model_path.exists():
                self.yolo_model = YOLO(str(yolo_model_path))
            else:
                self.yolo_model = YOLO(YOLO_MODEL_CONFIG["filename"])
            
            # CUDA 디바이스로 이동
            if self.detection_device == 'cuda':
                self.yolo_model.to('cuda')
            
            self.logger.info("✅ YOLO11L 모델 로드 완료")
            ...
            try:
                ...
                self.logger.info("✅ RTMW 모델 로드 완료")
            except ImportError as e:
                self.logger.error(f"❌ MMPose 임포트 실패: {e}")
                self.logger.info("pip install mmcv-full mmpose를 실행하세요.")
                raise
    ...
    def process_frame(self, frame):
        ...
        try:
            # YOLO로 사람 검출
            results = self.yolo_model(frame, classes=[0], conf=0.5, verbose=False)  # 0: person class
            ...
            for result in results:
                ...
                for box, score in zip(boxes, scores):
                    detected_results.append((None, score, box.tolist()))  # (keypoints, confidence, bbox)
            
            # 신뢰도 순으로 정렬
            detected_results.sort(key=lambda x: x[1], reverse=True)
            
            # 시각화 (선택사항)
            vis_frame = frame.copy()
            for _, score, bbox in detected_results[:1]:  # 가장 신뢰도 높은 1명만
                x1, y1, x2, y2 = map(int, bbox)
                cv2.rectangle(vis_frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                cv2.putText(vis_frame, f"Person: {score:.2f}", (x1, y1-10), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
            
            return vis_frame, detected_results
            
        except Exception as e:
            self.logger.warning(f"⚠️ 프레임 처리 실패: {e}")
            return frame, []
    
    def estimate_pose_on_crop(self, crop_image):
        """크롭된 이미지에서 포즈 추정"""
        try:
            ...
            # MMPose로 포즈 추정
            pose_results = inference_topdown(self.pose_model, crop_image, bbox)
            
            if not pose_results or len(pose_results) == 0:
                # 기본값 반환 (133개 키포인트, 좌표 0, 스코어 0)
                return np.zeros((133, 2), dtype=np.float32), np.zeros(133, dtype=np.float32)
            ...
            if 'keypoints' in pose_result:
                keypoints = pose_result['keypoints'][:, :2]  # x, y만 사용
                scores = pose_result['keypoints'][:, 2] if pose_result['keypoints'].shape[1] > 2 else np.ones(len(keypoints))
            else:
                keypoints = np.zeros((133, 2), dtype=np.float32)
                scores = np.zeros(133, dtype=np.float32)
            
            # 133개 키포인트로 맞추기 (패딩 또는 자르기)
            if len(keypoints) < 133:
                pad_size = 133 - len(keypoints)
                keypoints = np.vstack([keypoints, np.zeros((pad_size, 2), dtype=np.float32)])
                scores = np.concatenate([scores, np.zeros(pad_size, dtype=np.float32)])
            elif len(keypoints) > 133:
...
class StreamlinedVideoProcessor:
    """HDF5용 간소화된 비디오 처리기 (WORD + SEN 지원) - NVIDIA GPU 버전"""
    ...
    def __init__(self, 
        ...
        if device == "auto":
            if torch.cuda.is_available():
                self.device = "cuda"
                self.logger.info("✅ NVIDIA GPU 자동 감지됨")
            else:
                self.device = "cpu"
                self.logger.info("⚠️ GPU 미감지, CPU 사용")
        else:
            self.device = device
        ...
        # YOLO11L + RTMW 하이브리드 추론기 초기화 (NVIDIA GPU용)
        self.inferencer = YOLO11LCUDAHybridInferencer(
        ...
        self.logger.info("✅ 스트림라인 비디오 처리기 초기화 완료 (NVIDIA GPU)")
    ...
    def process_video_to_arrays(self, video_path: str) -> Optional[Dict[str, Union[List[np.ndarray], np.ndarray, int]]]:
        ...
        try:
            ...
            while True:
                ...
                try:
                    # GPU 메모리 정리 (매 100프레임마다)
                    if frame_idx % 100 == 0 and self.device == 'cuda':
    ...
    def process_video(self, item_type: str, item_id: int, video_path: str, output_dir: Path) -> bool:
        ...
        try:
            ...
            metadata = {
                ...
                'device': self.device,  # 사용된 디바이스 정보 추가
                'shape_info': {
                    'frame_count': arrays['frame_count'],
...
class BatchProcessor:
    """폴더별 250개 단위 배치 처리기 (WORD + SEN 지원) - NVIDIA GPU 버전"""
    ...
    def __init__(self, 
        ...
        self.logger.info(f"🎯 사용 디바이스: {device_info}")
        ...
        self.logger.info("✅ 배치 처리기 초기화 완료 (NVIDIA GPU)")
    ...
    def _get_device_info(self) -> str:
        ...
        if self.device == "auto":
            if torch.cuda.is_available():
                ...
                gpu_memory = torch.cuda.get_device_properties(0).total_memory / 1024**3
                return f"NVIDIA GPU ({gpu_name}, {gpu_memory:.1f}GB)"
            else:
                return "CPU (GPU 미감지)"
            ...
            if torch.cuda.is_available():
                ...
                return "CPU (CUDA 사용 불가)"
        else:
            return f"{self.device.upper()}"
    ...
    def process_batch(self, batch_info: Dict) -> List[str]:
        ...
        for item_type, item_id, video_path in tqdm(batch_data, desc=f"배치 {batch_id} [{folder_name}]"):
            ...
            # GPU 메모리 정리 (매 10개 비디오마다)
            if len(successful_keys) % 10 == 0 and self.device in ['cuda', 'auto']:
                torch.cuda.empty_cache()
                gc.collect()
    ...
    def process_test_batch(self, folder_name: str = None, test_count: int = 5):
        ...
        types_str = ", ".join(sorted(list(types_in_test)))
...
def main():
    """메인 실행 함수 - NVIDIA GPU 버전"""
    print("🚀 스트림라인 배치 처리기 (WORD + SEN 지원) - NVIDIA GPU 버전")
    print("=" * 60)
    ...
    if torch.cuda.is_available():
        gpu_name = torch.cuda.get_device_name()
        gpu_memory = torch.cuda.get_device_properties(0).total_memory / 1024**3
        print(f"🎯 감지된 GPU: {gpu_name} ({gpu_memory:.1f}GB)")
    else:
        print("⚠️  NVIDIA GPU를 감지할 수 없습니다. CPU로 실행됩니다.")
    
    # 디바이스 선택
    print("\n사용할 디바이스를 선택하세요:")
    print("1. 자동 감지 (기본값)")
    print("2. NVIDIA GPU 강제 사용")
    print("3. CPU 강제 사용")
