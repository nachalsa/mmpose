...
# 설정 및 MMPose 관련 임포트
from config import MODELS_DIR, YOLO_MODEL_CONFIG, RTMW_MODEL_OPTIONS, RTMW_ONNX_MODEL_OPTIONS
...
class StreamlinedVideoProcessor:
    ...
    def __init__(self, 
        ...
        # ONNX 모델 경로 설정
        rtmw_onnx_path = self._ensure_rtmw_onnx_model(rtmw_model_name)
    ...
    def _ensure_rtmw_onnx_model(self, model_name: str = "rtmw-l_simcc-cocktail14_pt-ucoco_270e-384x288.onnx") -> str:
        ...
        possible_paths = [
            ...
            Path("models") / model_name,
            Path(model_name)  # 현재 디렉토리
        ]
        
        # 기존 모델 파일 확인
        for model_path in possible_paths:
            if model_path.exists():
                self.logger.info(f"✅ 기존 RTMW ONNX 모델 발견: {model_path}")
        ...
        if model_name in onnx_model_urls:
            ...
            self.logger.error(f"❌ 알 수 없는 ONNX 모델명: {model_name}")
            return str(model_path)
    ...
    def process_video_to_arrays_batched(self, video_path: str, frame_batch_size: int = 16) -> Optional[Dict[str, Union[List[np.ndarray], np.ndarray, int]]]:
        ...
        try:
            ...
            while True:
                ...
                frame_batch.append((crop_image, keypoints, scores))
    ...
    def _process_frame_batch(self, frame_batch: List[Tuple], bbox_batch: List, 
                        all_jpeg_frames: List, all_keypoints: List, all_scores: List):
        """프레임 배치를 처리 (ONNX는 이미 개별 처리됨)"""
        try:
            # ONNX는 이미 process_frame에서 포즈 추정이 완료됨
            for crop_image, keypoints, scores in frame_batch:
                ret_jpg, encoded_jpg = cv2.imencode('.jpg', crop_image, [cv2.IMWRITE_JPEG_QUALITY, 90])
                if ret_jpg:
                    all_jpeg_frames.append(encoded_jpg)
                    all_keypoints.append(keypoints)
...
class BatchProcessor:
    """폴더별 250개 단위 배치 처리기 (ONNX 최적화 병렬 처리)"""
...
def main():
    ...
    yolo_choice = input("YOLO 디바이스 선택 (1-3, 기본값: 1): ").strip()
    yolo_device_map = {'1': 'auto', '2': 'cpu', '3': 'cuda', '': 'auto'}
    yolo_device = yolo_device_map.get(yolo_choice, 'auto')
    
    print("\nONNX 포즈 추정 디바이스를 선택하세요:")
    print("1. 자동 (기본값)")
    print("2. CPU")
    print("3. CUDA (NVIDIA GPU)")
    print("4. OpenVINO (Intel GPU/CPU)")
    
    pose_choice = input("포즈 디바이스 선택 (1-4, 기본값: 1): ").strip()
    pose_device_map = {'1': 'auto', '2': 'cpu', '3': 'cuda', '4': 'openvino', '': 'auto'}
    pose_device = pose_device_map.get(pose_choice, 'auto')
    
    print(f"✅ 선택된 디바이스: YOLO={yolo_device.upper()}, Pose={pose_device.upper()}")
    ...
    while True:
        ...
        if choice == '1':
            ...
            # ONNX 모델 확인
            found = False
            for config in RTMW_ONNX_MODEL_OPTIONS:
                if rtmw_model_name in config["filename"]:
                    rtmw_path = Path(config["path"])
                    print(f"  - RTMW ONNX 파일 존재: {'✅' if rtmw_path.exists() else '❌'}")
                    if rtmw_path.exists():
            ...
            # 기존 PyTorch 모델도 확인 (폴백용)
            if not found:
                for config in RTMW_MODEL_OPTIONS:
                    if rtmw_model_name in config["filename"]:
                        rtmw_path = Path(config["path"])
                        print(f"  - RTMW PyTorch 파일 존재: {'✅' if rtmw_path.exists() else '❌'}")
                        if rtmw_path.exists():
                            size_mb = rtmw_path.stat().st_size / (1024*1024)
                            print(f"    크기: {size_mb:.1f} MB")
                        break
