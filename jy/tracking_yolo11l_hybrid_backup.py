            ...
            while True:
                if not paused:
                    ...
                    if results:
                        ...
                        for i, (_, scores, _, track_id) in enumerate(results):
                            hand_kpts = np.sum(scores[91:133] > 0.5)
                            person_info += f" | ID{track_id}: {hand_kpts}/42 hands"
                        cv2.putText(vis_frame, person_info, 
                                   (10, info_y + 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
                ...
                # 화면 표시
                cv2.imshow('Real-time YOLO11L + RTMW Tracking (XPU)', vis_frame)
                ...
                if key == 27:  # ESC
                    break
                elif key == ord('r') or key == ord('R'):  # 리셋
                    self.reset_tracking()
                    print("🔄 트래킹 리셋됨")
                elif key == ord('s') or key == ord('S'):  # 스크린샷
                    screenshot_name = f"webcam_screenshot_{screenshot_count:04d}.jpg"
